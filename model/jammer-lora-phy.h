 ...
 */
class JammerLoraPhy : public LoraPhy
{
public:
  /**
   * An enumeration of the possible states of an JammerLoraPhy.
   ...
   * JammerLoraPhy is listening on, the packet will be discarded.
   *
 ...
 /**
   ...
   *
   * \return The state this JammerLoraPhy is currently in.
   */
  JammerLoraPhy::State GetState (void);
...
private:
   ...
   * the one this JammerLoraPhy was configured to listen for.
   */
  TracedCallback<Ptr<const Packet>, uint32_t> m_wrongSf;

  /**
   * Trace source for when a packet is lost because it was transmitted on a
   * frequency different from the one this JammerLoraPhy was configured to
   * listen on.
   */
